...
static void load_image(app_state& app, const char* path) {
    ...
    app.tex_image =
        upload_texture(app.image.data.data(), app.image.width, app.image.height, 3, app.tex_image);
...
static void run_pcs(app_state& app) {
    ...
    snprintf(
        app.status, sizeof(app.status), "PCS: %d detections.", (int) app.result.detections.size());
    app.busy = false;
    build_overlay(app);
}
...
static void run_pvs(app_state& app) {
    if (!app.image_encoded)
        return;
    if (app.pos_points.empty() && app.neg_points.empty())
        return;
    ...
    snprintf(app.status, sizeof(app.status), "PVS: %d masks.", (int) app.result.detections.size());
    app.busy = false;
    build_overlay(app);
}
...
static void run_pvs_box(app_state& app) {
    ...
    app.result = sam3_segment_pvs(*app.state, *app.model, pvs);
    snprintf(
        app.status, sizeof(app.status), "PVS (box): %d masks.", (int) app.result.detections.size());
    app.busy = false;
    build_overlay(app);
}
...
static bool screen_to_image(const app_state& app, float sx, float sy, float& ix, float& iy) {
...
static void draw_busy_overlay(const char* message) {
    ...
    for (int i = 0; i < num_segments; ++i) {
        ...
        ImGui::GetForegroundDrawList()->AddCircleFilled(
            ImVec2(x, y), 3.0f, IM_COL32(255, 255, 255, (int) (alpha * 255)));
    }

    // Text below spinner
    ImVec2 text_size = ImGui::CalcTextSize(message);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(center.x - text_size.x * 0.5f, center.y + 10.0f),
...
int main(int argc, char** argv) {
    ...
    if (app.visual_only) {
        ...
        fprintf(stderr,
                "Model: %s — PCS (text) mode disabled\n",
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_DROPFILE) {
                ...
                std::string title =
                    basename_of(dropped) + " \xe2\x80\x94 " + basename_of(app.params.model_path);
        ...
        if (!app.visual_only) {
            ...
            bool enter_pressed = ImGui::InputText("##prompt",
                                                  app.text_prompt,
        ...
        ImGui::Text("Mode:");
        ImGui::SameLine();
        if (ImGui::RadioButton("Points", app.mode == MODE_POINTS))
            app.mode = MODE_POINTS;
        ImGui::SameLine();
        if (ImGui::RadioButton("Box (PVS)", app.mode == MODE_BOX_PVS))
            app.mode = MODE_BOX_PVS;
        ImGui::SameLine();
        if (!app.visual_only) {
        ...
        if (!app.image.data.empty() && app.tex_overlay) {
            ...
            dl->AddImage(
                (ImTextureID) (intptr_t) app.tex_overlay, pos, ImVec2(pos.x + dw, pos.y + dh));
            ...
            for (const auto& b : app.pos_exemplars) {
                ...
                dl->AddRect(ImVec2(sx0, sy0), ImVec2(sx1, sy1), IM_COL32(0, 255, 0, 200), 0, 0, 2);
            }
    ...
    if (app.tex_image)
        glDeleteTextures(1, &app.tex_image);
    if (app.tex_overlay)
        glDeleteTextures(1, &app.tex_overlay);
