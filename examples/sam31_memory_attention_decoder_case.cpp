...
struct Args {
    ...
    std::string case_dir = "outputs/sam31-memory-attention-decoder-case";
    std::string output_dir = "outputs/sam31-memory-attention-decoder-cpp";
    int threads = 4;
    bool use_gpu = false;
};
...
int main(int argc, char** argv) {
    try {
        ...
        if (!sam3_test_dump_sam31_memory_attention_decoder_case(
                *model, args.case_dir, args.output_dir, args.threads)) {
            std::cerr << "SAM3.1 memory attention decoder case failed\n";
            return 1;
        }

        std::cout << std::format("wrote SAM3.1 propagation outputs to {}\n", args.output_dir);
    } catch (const std::exception& ex) {
        std::cerr << std::format("sam31_memory_attention_decoder_case failed: {}\n", ex.what());
        return 1;
    }
    return 0;
}
