...
static bool run_selective_vit(sam3_state& state,
    ...
    if (!sam3_encode_vit_from_preprocessed_selective(
...
static ref_tensor_f32 run_stage_or_die(const sam3_model& model,
                                       ...
                                       int n_threads) {
    std::array<int64_t, 4> in_ne = {1, 1, 1, 1};
    ...
    if (!sam3_test_run_vit_block_stage(
            model, 0, stage, input.data, in_ne, out.data, out_ne, n_threads)) {
...
static ref_tensor_f32 run_linear_host_ref_or_die(const sam3_model& model,
    ...
    if (!sam3_test_run_vit_block_linear_host_ref(
            model, 0, stage, input.data, in_ne, use_double_accum, out.data, out_ne)) {
...
static ref_tensor_f32 run_block0_input_or_die(const sam3_model& model,
    ...
    int64_t out_ne[4] = {0, 0, 0, 0};
    ref_tensor_f32 out;
    if (!sam3_test_run_vit_block0_input(
            model, chw_data.data(), img_size, out.data, out_ne, n_threads)) {
...
int main(int argc, char** argv) {
    ...
    if (!run_selective_vit(
            *cpu_state, *cpu_model, preprocessed.data, img_size, graph_items, cpu_dir)) {
        fprintf(stderr, "CPU selective block-0 run failed\n");
        return 1;
    }
    if (!run_selective_vit(
            *metal_state, *metal_model, preprocessed.data, img_size, graph_items, metal_dir)) {
    ...
    const ref_tensor_f32 cpu_block_input =
        run_block0_input_or_die(*cpu_model, preprocessed.data, img_size, n_threads);
    const ref_tensor_f32 metal_block_input =
        run_block0_input_or_die(*metal_model, preprocessed.data, img_size, n_threads);
    ...
    fprintf(
        stderr, "window attention: full_spatial=72x72 window=24 heads=%d head_dim=%d\n", 16, 64);
    fprintf(stderr,
            "qkv weight:  shape=[%lld,%lld] type=%d\n",
            (long long) qkv_info.ne[0],
            (long long) qkv_info.ne[1],
            qkv_info.type);
    fprintf(stderr,
            "proj weight: shape=[%lld,%lld] type=%d\n",
            (long long) proj_info.ne[0],
            (long long) proj_info.ne[1],
            proj_info.type);
    fprintf(stderr,
            "mlp fc1:     shape=[%lld,%lld] type=%d\n",
            (long long) mlp1_info.ne[0],
            (long long) mlp1_info.ne[1],
            mlp1_info.type);
    fprintf(stderr,
            "freqs_cis:   shape=[%lld,%lld,%lld] type=%d\n",
            (long long) rope_info.ne[0],
            (long long) rope_info.ne[1],
            (long long) rope_info.ne[2],
            rope_info.type);
    print_diff_row("block0_input_exact", cpu_block_input, metal_block_input);

    const ref_tensor_f32 cpu_norm1_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_NORM1, cpu_block_input, n_threads);
    const ref_tensor_f32 metal_norm1_self =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_NORM1, metal_block_input, n_threads);
    const ref_tensor_f32 cpu_win_part_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_WINDOW_PART, cpu_norm1_self, n_threads);
    const ref_tensor_f32 metal_win_part_self = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_WINDOW_PART, metal_norm1_self, n_threads);
    const ref_tensor_f32 cpu_qkv_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, cpu_win_part_self, n_threads);
    const ref_tensor_f32 metal_qkv_self = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, metal_win_part_self, n_threads);
    const ref_tensor_f32 cpu_attn_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_ATTN_CORE, cpu_qkv_self, n_threads);
    const ref_tensor_f32 metal_attn_self =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_ATTN_CORE, metal_qkv_self, n_threads);
    const ref_tensor_f32 cpu_attn_proj_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, cpu_attn_self, n_threads);
    const ref_tensor_f32 metal_attn_proj_self =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, metal_attn_self, n_threads);
    const ref_tensor_f32 cpu_unpart_self = run_stage_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART, cpu_attn_proj_self, n_threads);
    const ref_tensor_f32 metal_unpart_self = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART, metal_attn_proj_self, n_threads);
    ...
    const ref_tensor_f32 cpu_norm2_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_NORM2, cpu_resid1_self, n_threads);
    const ref_tensor_f32 metal_norm2_self =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_NORM2, metal_resid1_self, n_threads);
    const ref_tensor_f32 cpu_mlp_self =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_MLP, cpu_norm2_self, n_threads);
    const ref_tensor_f32 metal_mlp_self =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_MLP, metal_norm2_self, n_threads);
    ...
    const ref_tensor_f32 cpu_norm1_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_NORM1, cpu_block_input, n_threads);
    const ref_tensor_f32 metal_norm1_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_NORM1, cpu_block_input, n_threads);
    const ref_tensor_f32 cpu_win_part_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_WINDOW_PART, cpu_norm1_shared, n_threads);
    const ref_tensor_f32 metal_win_part_shared = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_WINDOW_PART, cpu_norm1_shared, n_threads);
    const ref_tensor_f32 cpu_qkv_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, cpu_win_part_shared, n_threads);
    const ref_tensor_f32 metal_qkv_shared = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, cpu_win_part_shared, n_threads);
    const ref_tensor_f32 cpu_attn_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_ATTN_CORE, cpu_qkv_shared, n_threads);
    const ref_tensor_f32 metal_attn_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_ATTN_CORE, cpu_qkv_shared, n_threads);
    const ref_tensor_f32 cpu_attn_proj_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, cpu_attn_shared, n_threads);
    const ref_tensor_f32 metal_attn_proj_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, cpu_attn_shared, n_threads);
    const ref_tensor_f32 cpu_unpart_shared = run_stage_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART, cpu_attn_proj_shared, n_threads);
    const ref_tensor_f32 metal_unpart_shared = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART, cpu_attn_proj_shared, n_threads);
    ...
    const ref_tensor_f32 cpu_norm2_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_NORM2, cpu_resid1_shared, n_threads);
    const ref_tensor_f32 metal_norm2_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_NORM2, cpu_resid1_shared, n_threads);
    const ref_tensor_f32 cpu_mlp_fc1_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_MLP_FC1, cpu_norm2_shared, n_threads);
    const ref_tensor_f32 metal_mlp_fc1_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_MLP_FC1, cpu_norm2_shared, n_threads);
    const ref_tensor_f32 cpu_mlp_gelu_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_MLP_GELU, cpu_mlp_fc1_shared, n_threads);
    const ref_tensor_f32 metal_mlp_gelu_shared = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_MLP_GELU, cpu_mlp_fc1_shared, n_threads);
    const ref_tensor_f32 cpu_mlp_fc2_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_MLP_FC2, cpu_mlp_gelu_shared, n_threads);
    const ref_tensor_f32 metal_mlp_fc2_shared = run_stage_or_die(
        *metal_model, SAM3_VIT_BLOCK_STAGE_MLP_FC2, cpu_mlp_gelu_shared, n_threads);
    const ref_tensor_f32 cpu_mlp_shared =
        run_stage_or_die(*cpu_model, SAM3_VIT_BLOCK_STAGE_MLP, cpu_norm2_shared, n_threads);
    const ref_tensor_f32 metal_mlp_shared =
        run_stage_or_die(*metal_model, SAM3_VIT_BLOCK_STAGE_MLP, cpu_norm2_shared, n_threads);
    ...
    const ref_tensor_f32 host_qkv_f64 = run_linear_host_ref_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, cpu_win_part_shared, true);
    const ref_tensor_f32 host_attn_proj_f64 = run_linear_host_ref_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, cpu_attn_shared, true);
    const ref_tensor_f32 host_mlp_fc1_f64 = run_linear_host_ref_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_MLP_FC1, cpu_norm2_shared, true);
    const ref_tensor_f32 host_mlp_fc2_f64 = run_linear_host_ref_or_die(
        *cpu_model, SAM3_VIT_BLOCK_STAGE_MLP_FC2, cpu_mlp_gelu_shared, true);
    ...
    fprintf(stderr,
            ...
            "stage",
            "shape",
            "cpu_iso_vs_cpu_graph",
            "metal_iso_vs_metal_graph");
    ...
    auto print_self_row = [&](const char* label,
        ...
        const compare_result cpu_r = compare_tensors(
            cpu_iso.data.data(), cpu_it->second.data.data(), cpu_iso.numel(), 1e-4f);
        const compare_result metal_r = compare_tensors(
            metal_iso.data.data(), metal_it->second.data.data(), metal_iso.numel(), 1e-4f);
    ...
    print_self_row(
        "attn_proj_output", cpu_attn_proj_self, metal_attn_proj_self, "attn_proj_output");
