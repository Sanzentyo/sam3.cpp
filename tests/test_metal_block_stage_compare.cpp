...
static ref_tensor_f32 run_stage_or_die(const sam3_model& model,
    ...
    if (!sam3_test_run_vit_block_stage(
            model, block_idx, stage, input.data, in_ne, out.data, out_ne, n_threads)) {
...
static ref_tensor_f32 run_linear_host_ref_or_die(const sam3_model& model,
    ...
    if (!sam3_test_run_vit_block_linear_host_ref(
            model, block_idx, stage, input.data, in_ne, use_double_accum, out.data, out_ne)) {
...
static stage_bundle run_block_bundle(const sam3_model& model,
    ...
    out.norm1 =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_NORM1, block_input, n_threads);
    out.attn_input =
        is_global ? out.norm1
                  : run_stage_or_die(
                        model, block_idx, SAM3_VIT_BLOCK_STAGE_WINDOW_PART, out.norm1, n_threads);
    out.qkv = run_stage_or_die(
        model, block_idx, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, out.attn_input, n_threads);
    out.attn =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_ATTN_CORE, out.qkv, n_threads);
    out.attn_proj =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, out.attn, n_threads);
    out.post_attn =
        is_global
            ? out.attn_proj
            : run_stage_or_die(
                  model, block_idx, SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART, out.attn_proj, n_threads);
    out.resid1 = add_ref(block_input, out.post_attn);
    out.norm2 =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_NORM2, out.resid1, n_threads);
    out.mlp_fc1 =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC1, out.norm2, n_threads);
    out.mlp_gelu =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_GELU, out.mlp_fc1, n_threads);
    out.mlp_fc2 =
        run_stage_or_die(model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC2, out.mlp_gelu, n_threads);
...
int main(int argc, char** argv) {
    ...
    const std::string input_name =
        block_idx == 0 ? "dbg_after_ln_pre" : make_block_name(block_idx - 1, "out");
    ...
    if (!run_selective_vit(
            *cpu_state, *cpu_model, preprocessed.data, img_size, block_idx, graph_items, cpu_dir)) {
        fprintf(stderr, "CPU selective block run failed\n");
        return 1;
    }
    if (!run_selective_vit(*metal_state,
                           *metal_model,
                           preprocessed.data,
                           img_size,
                           block_idx,
    ...
    sam3_get_model_tensor_info(
        *cpu_model, "vit.blocks." + std::to_string(block_idx) + ".attn.qkv.weight", qkv_info);
    sam3_get_model_tensor_info(
        *cpu_model, "vit.blocks." + std::to_string(block_idx) + ".attn.proj.weight", proj_info);
    sam3_get_model_tensor_info(
        *cpu_model, "vit.blocks." + std::to_string(block_idx) + ".mlp.lin1.weight", mlp1_info);
    ...
    fprintf(stderr,
            ...
            proj_info.type);
    fprintf(stderr,
            "mlp fc1 weight:    shape=[%lld,%lld] type=%d\n",
    ...
    const stage_bundle cpu_shared =
        run_block_bundle(*cpu_model, block_idx, global, cpu_block_input, n_threads);
    const stage_bundle metal_shared =
        run_block_bundle(*metal_model, block_idx, global, cpu_block_input, n_threads);
    const stage_bundle cpu_self =
        run_block_bundle(*cpu_model, block_idx, global, cpu_block_input, n_threads);
    const stage_bundle metal_self =
        run_block_bundle(*metal_model, block_idx, global, metal_block_input, n_threads);
    ...
    print_diff_row(global ? "attn_input_output" : "window_part_output",
                   cpu_shared.attn_input,
                   metal_shared.attn_input);
    const compare_result qkv_diff =
        print_diff_row_ex("qkv_proj_output", cpu_shared.qkv, metal_shared.qkv);
    const compare_result attn_diff =
        print_diff_row_ex("attn_core_output", cpu_shared.attn, metal_shared.attn);
    const compare_result attn_proj_diff =
        print_diff_row_ex("attn_proj_output", cpu_shared.attn_proj, metal_shared.attn_proj);
    print_diff_row(global ? "post_attn_output" : "window_unpart_output",
                   cpu_shared.post_attn,
                   metal_shared.post_attn);
    ...
    const compare_result mlp_fc2_diff =
        print_diff_row_ex("mlp_fc2_output", cpu_shared.mlp_fc2, metal_shared.mlp_fc2);
    print_diff_row("mlp_output", cpu_shared.mlp, metal_shared.mlp);
    print_diff_row("block_output", cpu_shared.block, metal_shared.block);

    auto print_worst_loc =
        [](const char* label, const ref_tensor_f32& t, const compare_result& diff) {
            ...
            fprintf(stderr,
                    "%s worst: col=%d row=%d cpu=%.6f metal=%.6f max_abs=%.6f\n",
                    label,
                    worst_col,
                    worst_row,
                    diff.worst_a,
    ...
    const ref_tensor_f32 qkv_in_slice =
        slice_first_columns(cpu_shared.attn_input, linear_slice_cols);
    ...
    const ref_tensor_f32 attn_proj_in_slice =
        slice_first_columns(cpu_shared.attn, linear_slice_cols);
    const ref_tensor_f32 attn_proj_cpu_slice =
        slice_first_columns(cpu_shared.attn_proj, linear_slice_cols);
    const ref_tensor_f32 attn_proj_mtl_slice =
        slice_first_columns(metal_shared.attn_proj, linear_slice_cols);
    const ref_tensor_f32 mlp_fc1_in_slice =
        slice_first_columns(cpu_shared.norm2, linear_slice_cols);
    const ref_tensor_f32 mlp_fc1_cpu_slice =
        slice_first_columns(cpu_shared.mlp_fc1, linear_slice_cols);
    const ref_tensor_f32 mlp_fc1_mtl_slice =
        slice_first_columns(metal_shared.mlp_fc1, linear_slice_cols);
    const ref_tensor_f32 mlp_fc2_in_slice =
        slice_first_columns(cpu_shared.mlp_gelu, linear_slice_cols);
    const ref_tensor_f32 mlp_fc2_cpu_slice =
        slice_first_columns(cpu_shared.mlp_fc2, linear_slice_cols);
    const ref_tensor_f32 mlp_fc2_mtl_slice =
        slice_first_columns(metal_shared.mlp_fc2, linear_slice_cols);

    const ref_tensor_f32 host_qkv_f32 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, qkv_in_slice, false);
    const ref_tensor_f32 host_qkv_f64 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, qkv_in_slice, true);
    const ref_tensor_f32 host_attn_proj_f32 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, attn_proj_in_slice, false);
    const ref_tensor_f32 host_attn_proj_f64 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, attn_proj_in_slice, true);
    const ref_tensor_f32 host_mlp_fc1_f32 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC1, mlp_fc1_in_slice, false);
    const ref_tensor_f32 host_mlp_fc1_f64 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC1, mlp_fc1_in_slice, true);
    const ref_tensor_f32 host_mlp_fc2_f32 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC2, mlp_fc2_in_slice, false);
    const ref_tensor_f32 host_mlp_fc2_f64 = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC2, mlp_fc2_in_slice, true);
    ...
    const ref_tensor_f32 qkv_host_worst = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_QKV_PROJ, qkv_in_worst, false);

    const ref_tensor_f32 attn_proj_in_worst =
        slice_column_range(cpu_shared.attn, attn_proj_worst_col, 1);
    const ref_tensor_f32 attn_proj_cpu_worst =
        slice_column_range(cpu_shared.attn_proj, attn_proj_worst_col, 1);
    const ref_tensor_f32 attn_proj_mtl_worst =
        slice_column_range(metal_shared.attn_proj, attn_proj_worst_col, 1);
    const ref_tensor_f32 attn_proj_host_worst = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_ATTN_PROJ, attn_proj_in_worst, false);

    const ref_tensor_f32 mlp_fc2_in_worst =
        slice_column_range(cpu_shared.mlp_gelu, mlp_fc2_worst_col, 1);
    const ref_tensor_f32 mlp_fc2_cpu_worst =
        slice_column_range(cpu_shared.mlp_fc2, mlp_fc2_worst_col, 1);
    const ref_tensor_f32 mlp_fc2_mtl_worst =
        slice_column_range(metal_shared.mlp_fc2, mlp_fc2_worst_col, 1);
    const ref_tensor_f32 mlp_fc2_host_worst = run_linear_host_ref_or_die(
        *cpu_model, block_idx, SAM3_VIT_BLOCK_STAGE_MLP_FC2, mlp_fc2_in_worst, false);
    ...
    const std::map<std::string, ref_tensor_f32> metal_graph = {
        ...
        {"attn_proj_output",
         load_ref_f32(metal_dir + "/" + make_block_name(block_idx, "attn_proj"))},
    ...
    print_self_row(
        "attn_proj_output", cpu_self.attn_proj, metal_self.attn_proj, "attn_proj_output");
