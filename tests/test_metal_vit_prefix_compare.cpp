...
static ref_tensor_f32 run_prefix_stage_or_die(const sam3_model& model,
    ...
    if (!sam3_test_run_vit_prefix_stage(
            model, stage, input.data, in_ne, out.data, out_ne, n_threads)) {
...
static ref_tensor_f32 run_block0_input_or_die(const sam3_model& model,
    ...
    if (!sam3_test_run_vit_block0_input(
            model, image.data.data(), img_size, out.data, out_ne, n_threads)) {
...
int main(int argc, char** argv) {
    ...
    const ref_tensor_f32 cpu_im2col = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_PATCH_IM2COL, image_input, n_threads);
    const ref_tensor_f32 metal_im2col = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_PATCH_IM2COL, image_input, n_threads);
    print_diff_row("patch_im2col", cpu_im2col, metal_im2col);

    const ref_tensor_f32 cpu_patch_raw_shared = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT_RAW, cpu_im2col, n_threads);
    const ref_tensor_f32 metal_patch_raw_shared = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT_RAW, cpu_im2col, n_threads);
...

    {
        ...
        if (!sam3_test_run_patch_mulmat_host_ref(
                *cpu_model, cpu_im2col.data, in_ne, false, host_patch_raw_f32.data, out_ne_f32) ||
            !sam3_test_run_patch_mulmat_host_ref(
                *cpu_model, cpu_im2col.data, in_ne, true, host_patch_raw_f64.data, out_ne_f64)) {
    ...
    const ref_tensor_f32 cpu_patch_shared = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT, cpu_im2col, n_threads);
    const ref_tensor_f32 metal_patch_shared = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT, cpu_im2col, n_threads);
    print_diff_row("patch_mulmat", cpu_patch_shared, metal_patch_shared);

    const ref_tensor_f32 cpu_patch = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_PATCH_EMBED, image_input, n_threads);
    const ref_tensor_f32 metal_patch = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_PATCH_EMBED, image_input, n_threads);
    print_diff_row("patch_embed", cpu_patch, metal_patch);

    const ref_tensor_f32 cpu_pos_shared =
        run_prefix_stage_or_die(*cpu_model, SAM3_VIT_PREFIX_STAGE_POS_ADD, cpu_patch, n_threads);
    const ref_tensor_f32 metal_pos_shared =
        run_prefix_stage_or_die(*metal_model, SAM3_VIT_PREFIX_STAGE_POS_ADD, cpu_patch, n_threads);
    print_diff_row("after_pos_embed", cpu_pos_shared, metal_pos_shared);

    const ref_tensor_f32 cpu_ln_norm_shared = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_LN_PRE_NORM, cpu_pos_shared, n_threads);
    const ref_tensor_f32 metal_ln_norm_shared = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_LN_PRE_NORM, cpu_pos_shared, n_threads);
    print_diff_row("ln_pre_norm", cpu_ln_norm_shared, metal_ln_norm_shared);

    const ref_tensor_f32 cpu_ln_pre_shared = run_prefix_stage_or_die(
        *cpu_model, SAM3_VIT_PREFIX_STAGE_LN_PRE, cpu_pos_shared, n_threads);
    const ref_tensor_f32 metal_ln_pre_shared = run_prefix_stage_or_die(
        *metal_model, SAM3_VIT_PREFIX_STAGE_LN_PRE, cpu_pos_shared, n_threads);
    print_diff_row("after_ln_pre", cpu_ln_pre_shared, metal_ln_pre_shared);

    const ref_tensor_f32 cpu_block0_input =
        run_block0_input_or_die(*cpu_model, preprocessed, img_size, n_threads);
    const ref_tensor_f32 metal_block0_input =
        run_block0_input_or_die(*metal_model, preprocessed, img_size, n_threads);
